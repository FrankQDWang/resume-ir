...
`GOAL.md`. It is intentionally stricter than the S0-S20 long-running execution
...
- S1-S20 foundation code has been rebuilt in the current worktree: `Cargo.toml`,
  ...
  email/phone payloads, or raw resume text, plus S17 metadata schema v3 durable
  import-task claim/lease/retry primitives that keep root paths, claim tokens,
  and local failure diagnostics out of public debug/status output, plus S18
  shared local import-worker code and daemon `--once` processing of one queued
  synthetic import root with aggregate-only daemon output, plus S19 bounded
  daemon foreground-once draining over all initially queued import tasks without
  same-run retry loops, plus S20 real subprocess daemon kill/recovery smoke that
  verifies an expired durable import-task lease is recovered on a later daemon
  run without leaking paths, file names, raw text, claim tokens, or diagnostics,
  with fresh per-task lease timestamps during one bounded drain.
...
- `PROGRESS.md` records S0 through S20 as complete while P0/P1/P2/P3/P6 remain
...
|---|---|---|---|---|
| P0 architecture skeleton | Design baseline, Git repo, README, PROGRESS, `.gitignore`; S1-S20 workspace/domain/config/SQLite metadata/CLI/daemon/fs-crawler/parser/text-processing/fulltext/import-smoke/field-filter/fake-semantic-interface/OCR-queue/doctor-diagnostics/delete-propagation/synthetic-benchmark/local-redacted-diagnostics-package/import-task-lease/shared-import-worker/bounded-daemon-drain/real-subprocess-recovery-smoke foundation has local acceptance evidence. | IPC, production logs/observability, CI, long-running daemon loop, production async import/index/delete orchestration, and broader fault-injection coverage. | Previous skeleton code was deleted and must not count as product progress; future code must carry fresh verification evidence. | Rust must be invoked with the user cargo path unless shell PATH is fixed; local CI/branch protection cannot be fully verified without remote setup. |
| P1 text import and full-text search | Product design for docx/PDF text, normalization, sectioning, Tantivy, snippets; S5 filesystem crawler exists with synthetic tests; S6 adds parser contracts, basic DOCX extraction, and honest PDF text-layer/image-only/unknown classification; S7 adds basic text cleanup, offset mapping, and heading/fallback sectioning; S8 adds real Tantivy indexing/search and CLI search over an existing index; S9 adds a synchronous synthetic DOCX/PDF fixture import loop that persists metadata/index state and searches after reader reopen; S14 adds `resume-cli delete --doc-id` tombstoning, committed Tantivy doc-id deletion when an index exists, no-index deletion without creating an index, tombstone-preserving re-import skip behavior, SQLite `DELETE_PENDING`/`DELETED`/`DELETE_ERROR` index state, and metadata-filtered search hiding for stale full-text hits after delete errors; S15 adds a local synthetic benchmark runner that exercises direct synthetic metadata/Tantivy indexing with batched metadata writes, metadata-gated search, delete, and post-delete search verification; S17 adds durable import-task claim/lease/retry primitives; S18 adds daemon `--once` processing of one queued synthetic import root through the shared local import worker; S19 drains all initially queued import roots once per foreground-once run without same-run retry loops; S20 verifies expired-lease recovery after a real daemon subprocess is killed post-claim. | Production long-running daemon import loop, broader PDF text extraction, async deletion propagation, and real 100k text import/delete benchmark runs. | The S9 importer, S14 deleter, S15 benchmark runner, S17 lease primitives, S18 one-task daemon drain, S19 bounded foreground-once drain, and S20 subprocess recovery smoke are CLI/local synthetic smoke, tooling, or metadata foundations, not the final asynchronous import/delete worker orchestration or a production benchmark result. | Large synthetic or desensitized corpora are not present. |
...
6. Continue P6 beyond the S13/S15/S16 local diagnostics and benchmark tooling plus S20 subprocess kill/recovery smoke with real production observability, recorded 100k/1M benchmark runs, broader fault injection, and soak only with real binaries, platform runners, and explicit signing/release approval.
