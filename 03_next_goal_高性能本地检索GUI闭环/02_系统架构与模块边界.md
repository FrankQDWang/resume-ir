...
8. 性能优化必须先满足 `15_性能观测与Profiling工具链.md` 的 baseline/profile/histogram 顺序。
9. 全盘发现和增量 journal 必须遵守 `16_跨平台全盘发现与增量Journal契约.md`，watcher event 只能作为信号，不能作为事实源。
10. 当前目标和验收红线必须能由 `ACTIVE_GOAL.toml` 与 `perf/acceptance-matrix.toml` 机器读取。
