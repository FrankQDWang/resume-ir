...
| Profile | 目标机器 | interactive 保护 | 后台策略 |
|---|---|---|---|
| `H0_weak` | 低配/节能/HDD/高 memory pressure | keyword/field search 优先，benchmark 默认拒绝或排队 | OCR/vector/import 严格限流，batch 小，必要时只做 Level1/Level2 |
| `H1_balanced` | 默认本机开发/办公机器 | interactive P95 和 queue delay 受 `perf/acceptance-matrix.toml` 约束 | 后台正常吞吐，遇到 overload 先暂停 benchmark/background |
| `H2_high` | 高配/接电/SSD/资源充足 | 仍不得突破 IPC deadline | 允许更高 OCR/vector/import 并发，但需要同样的 redacted aggregate evidence |
