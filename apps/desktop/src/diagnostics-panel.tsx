  ...
  return <div className="sheet-scroll diagnostics-content">
    ...
    {diagnostics && <>
      ...
      <section className="panel-card"><header><strong>本地聚合</strong><span>{diagnostics.evidence_lane} · {diagnostics.evidence_status} · epoch {countLabel(diagnostics.visible_epoch)}</span></header><dl>
        ...
        <div><dt>授权目录（在线 / 离线）</dt><dd>{countLabel(diagnostics.metrics.source_roots_total)}（{countLabel(diagnostics.metrics.source_roots_active)} / {countLabel(diagnostics.metrics.source_roots_offline)}）</dd></div>
        <div><dt>目录数据清理</dt><dd>{countLabel(diagnostics.metrics.source_root_deletions_in_progress)}</dd></div>
