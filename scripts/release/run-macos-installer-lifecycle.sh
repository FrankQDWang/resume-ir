...
usage() {
  cat >&2 <<'EOF'
usage: scripts/release/run-macos-installer-lifecycle.sh --version vX.Y.Z --macos-package-manifest FILE --out FILE --dry-run

Create a redacted macOS installer lifecycle dry-run plan. This does not run
installer, pkgutil, hdiutil, launchctl, install, upgrade, uninstall, rollback,
or start/stop a LaunchAgent.
EOF
}
...
[ -n "$out_file" ] || fail "missing --out"
[ "$dry_run" = "true" ] || fail "only --dry-run is supported without release-runner approval"
...
python3 - "$version" "$package_manifest" "$tmp_file" <<'PY'
import hashlib
import json
import os
import re
...
if seen_kinds != {"pkg", "dmg"}:
    fail("macOS package manifest must contain both pkg and dmg artifacts")

pkg_file = next(record["file"] for record in artifact_records if record["kind"] == "pkg")
dmg_file = next(record["file"] for record in artifact_records if record["kind"] == "dmg")

planned_actions = [
    {
        "action": "install",
        "command": "installer",
        "target_artifact": pkg_file,
        "dry_run_intent": "validate administrator-elevated pkg install on release runner",
    ...
    },
    {
        ...
        "command": "pkgutil",
        "target_artifact": pkg_file,
        "dry_run_intent": "forget package receipt and remove installed files while preserving user data",
        "requires_approval": True,
        "action_status": "blocked",
    },
    {
        ...
        "command": "installer",
        "target_artifact": pkg_file,
        "dry_run_intent": "force installer failure and verify system state restoration",
        "requires_approval": True,
        "action_status": "blocked",
    },
    {
        ...
        "command": "launchctl",
        "target_artifact": dmg_file,
        "dry_run_intent": "bootstrap LaunchAgent and verify daemon IPC health",
        "requires_approval": True,
        "action_status": "blocked",
    },
    {
        ...
        "command": "launchctl",
        "target_artifact": dmg_file,
        "dry_run_intent": "stop and bootout LaunchAgent and verify daemon shutdown",
        "requires_approval": True,
        "action_status": "blocked",
    },
]
...
document = {
    ...
    "notes": (
        "Dry-run operator plan only. It does not execute installer lifecycle "
        "commands or clear release blockers; release-runner transcripts are "
