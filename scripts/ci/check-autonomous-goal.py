...
def main() -> int:
    ...
    require_string(active_slice.get("issue"), "#145", "scope.active_slice.issue")
    require_string(
        active_slice.get("name"),
        "atomic_bootstrap_parse_result_cancel_poll_recovery",
    ...
    expected_allowed_paths = ["crates/import-pipeline/src/lib.rs", "PROGRESS.md"]
    ...
    bootstrap_transition = require_transition(transitions, "bootstrap_contract_and_test_fix")
    expected_bootstrap = {
        ...
        "required_evidence": ["explicit_user_authorization", "contract_conflict_evidence", "linked_issue", "issue_intent_comment", "exact_test_only_patch", "local_tests", "privacy_boundary"],
    ...
    for key, expected in expected_bootstrap.items():
        if bootstrap_transition.get(key) != expected:
            fail(
                "autonomous_delivery.transitions.bootstrap_contract_and_test_fix."
