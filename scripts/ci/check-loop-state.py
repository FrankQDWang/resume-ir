    ...
    if active_issue == "#145":
        ...
        if active_prs != ["#142", "#144"]:
            fail(
                ...
                "#145 authorization snapshot must keep open PRs #142 and #144"
            )
        open_blockers = github_ledger.get("open_blockers")
        required_blockers = {"#140", "#143", "#144", "#145"}
        if not isinstance(open_blockers, list) or not required_blockers.issubset(open_blockers):
            fail(
                "perf/current-loop-state.json.github_ledger.open_blockers: "
                "#145 authorization snapshot must retain #140/#143/#144/#145"
            )
