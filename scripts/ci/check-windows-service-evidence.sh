  ...
  "runtime_payload": {
    ...
    "components": [
      ...
      },
      {
        ...
        "kind": "pdf-renderer",
        "file": "resume-pdf-render-runtime.exe",
...
document["artifacts"][0]["local_probe_path"] = "PRIVATE-windows-service-cache"
...
if "$service_script" --version v0.0.0 --windows-package-manifest "$unknown_manifest" --out-dir "$out_dir/unknown" >/dev/null 2>&1; then
  fail "Windows service evidence script accepted an unknown Windows package manifest field"
fi
