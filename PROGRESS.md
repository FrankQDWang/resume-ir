...
- Product rule: S0-S20 slice progress is not the same as full product completion; P0-P6 gates remain authoritative.
...
|---|---|---|---|
| P0 architecture skeleton | In progress | Documentation baseline exists; S1-S20 foundation acceptance passed locally on 2026-05-31, including the S13 CLI doctor/diagnostics skeleton, S14 deletion-propagation CLI slice, S15 local synthetic benchmark runner with batched metadata seeding, S16 local redacted diagnostics package generation, S17 durable import-task lease/retry metadata schema v3, S18 shared import worker with daemon `--once` queued-import drain, S19 bounded daemon drain over initially queued import tasks, and S20 real subprocess kill/recovery smoke for daemon import-task leases. | Rust is installed under `/Users/frankqdwang/.cargo/bin` but not on default `PATH`; IPC, production logs/observability, CI, long-running daemon loop, and production async import orchestration remain unfinished. |
| P1 text import and full-text search | In progress | S5 crawler, S6 parser crates, S7 text normalization/sectioning, S8 Tantivy full-text index/search, S9 synthetic import-to-search smoke, S14 CLI `delete --doc-id` propagation, S15 benchmark search/delete smoke, S17 durable import-task claim/lease/retry primitives, S18 daemon `--once` processing of one queued synthetic import root, S19 bounded drain over initially queued import tasks, and S20 expired-lease recovery across real daemon subprocess death exist with acceptance tests. | Production long-running import worker, robust PDF extraction, synthetic large corpus, async deletion orchestration, and real benchmark corpus runs remain absent. |
...
| P6 performance and stability | In progress (synthetic smoke/scale tooling plus local redacted diagnostics package and one real subprocess fault-injection smoke only) | S13 adds a one-query small-data doctor smoke, redacted missing/corrupt full-text index status, helper-level simulated daemon-kill/disk-full diagnostics tests, and a redacted `export-diagnostics` skeleton. S15 adds `resume-cli benchmark --synthetic-count <n> --query <query>` for local synthetic metadata/Tantivy indexing with batched SQLite writes, metadata-gated search, existing delete-path verification, aggregate metrics, and scratch cleanup on success and handled failure paths. S16 adds `resume-cli export-diagnostics --redact --output <dir>` local package generation with aggregate-only `manifest.json`, `status.txt`, and `checks.txt` contents plus redacted stdout/errors. S20 kills a real `resume-daemon --foreground --once` subprocess after it has durably claimed a synthetic import task, then verifies a later daemon run expires and recovers the lease without leaking paths, file names, raw text, claim tokens, or local diagnostics. | No real 100k/1M benchmark result has been run or claimed here. Restart/recovery soak, production observability, corpus, query set, disk-full/file-lock fault injection, and platform runners remain absent. S16 is local redacted package generation only and S20 is one deterministic local subprocess smoke only; neither completes P6. |
...
| S19 | Complete | Extended `resume-daemon --foreground --once` to snapshot queued import task IDs at run start, attempt each initial task once via S17 `claim_import_task`, process claimed roots through `import-worker`, aggregate successful import counts, report `failed imports`, and avoid same-run retry loops for failed tasks. Acceptance commands passed locally on 2026-05-31. | This is bounded foreground-once draining only. It is not a long-running daemon loop, filesystem watcher, scheduler/backpressure policy, async deletion orchestration, OCR execution, semantic indexing, or production P0/P1 completion. |
| S20 | Complete | Added a real subprocess recovery smoke for `resume-daemon --foreground --once`: the test creates synthetic local import files, starts the actual daemon binary, uses a debug-build-only hook to pause after durable claim, kills the child process, waits for the short test lease to expire, runs the daemon binary again, and verifies the import task is recovered and completed with aggregate-only output. `meta-store` now exposes `claimable_import_tasks(now_ms)`, which expires stale running leases before returning a bounded start-of-run eligible task snapshot; daemon claims now compute fresh per-task lease timestamps during a drain. Acceptance commands passed locally on 2026-05-31. | This is one deterministic local subprocess fault-injection smoke only. It is not a long-running daemon loop, restart/recovery soak, disk-full/file-lock fault injection, production observability, OCR execution, semantic indexing, or real 100k/1M benchmark evidence. |
...
```bash
/Users/frankqdwang/.cargo/bin/cargo test -p resume-daemon foreground_once_drains_all_initially_queued_imports -- --nocapture
/Users/frankqdwang/.cargo/bin/cargo fmt --check
/Users/frankqdwang/.cargo/bin/cargo test -p resume-daemon
/Users/frankqdwang/.cargo/bin/cargo test --workspace
/Users/frankqdwang/.cargo/bin/cargo clippy --all-targets --all-features -- -D warnings
...
- TDD red run for `foreground_once_drains_all_initially_queued_imports` failed before implementation because daemon `--foreground --once` still printed `claimed imports: 1` instead of draining both initially queued import tasks.
- Final `cargo fmt --check`: succeeded.
- Final `cargo test -p resume-daemon`: succeeded with 7 tests covering foreground lifecycle, one queued import drain, bounded draining of all initially queued import tasks, aggregate-only output, retryable failed import drain, stale/reclaimed claim handling, and no same-run retry loop.
- Final `cargo test --workspace`: succeeded across all crates.
- Final `cargo clippy --all-targets --all-features -- -D warnings`: succeeded.
- Final `git diff --check`: succeeded.
- Final `git status --short --ignored -- local-data`: showed only `!! local-data/`.
...
- S19 changes daemon `--foreground --once` from one-task drain to bounded drain over the queued import task IDs visible at run start.
- Each initial task is attempted at most once in the run through `claim_import_task(task_id, ...)`; retryable failures are not re-claimed again during the same foreground-once invocation.
- Successful import summaries are aggregated across claimed tasks, while worker failures increment `failed imports` and are recorded through the generic redacted failure path.
- Daemon output remains aggregate-only and does not print roots, normalized paths, file names, raw text, claim tokens, or local diagnostics.
- S19 is still a bounded foreground-once local drain. It is not a long-running daemon loop, scheduler/backpressure implementation, filesystem watcher, OCR execution path, semantic/vector indexing path, or production performance result.

### S20

```bash
/Users/frankqdwang/.cargo/bin/cargo test -p resume-daemon killed_daemon_subprocess_recovers_expired_import_claim_on_next_run
/Users/frankqdwang/.cargo/bin/cargo test -p resume-daemon each_import_claim_gets_a_fresh_lease_during_one_drain
/Users/frankqdwang/.cargo/bin/cargo test -p meta-store claimable_import_tasks_expire_stale_running_leases_without_paths
/Users/frankqdwang/.cargo/bin/cargo fmt --check
/Users/frankqdwang/.cargo/bin/cargo test -p resume-daemon
/Users/frankqdwang/.cargo/bin/cargo test --workspace
/Users/frankqdwang/.cargo/bin/cargo clippy --all-targets --all-features -- -D warnings
...
- TDD red run for `killed_daemon_subprocess_recovers_expired_import_claim_on_next_run` first failed because the real daemon subprocess never reported a post-claim pause marker; after adding the debug-build-only pause/short-lease hook, the same test failed again because the second daemon run claimed `0` imports from an expired `running` lease.
- The recovery test passed after daemon start-of-run task selection began using `claimable_import_tasks(now_ms)`, which expires stale running import-task leases before returning a bounded eligible task snapshot.
- Quality-review regression `each_import_claim_gets_a_fresh_lease_during_one_drain` failed before the fix because a later task in the same drain reused an old lease and could be reclaimed immediately; it passed after daemon claims recomputed `now_ms` and lease expiry per task.
- Focused `cargo test -p meta-store claimable_import_tasks_expire_stale_running_leases_without_paths`: succeeded, covering expired running lease transition to retryable failed state, unexpired running exclusion, queued inclusion, and redacted Debug output.
- Final `cargo fmt --check`: succeeded.
- Final `cargo test -p resume-daemon`: succeeded with 8 tests, including the real subprocess kill/recovery smoke.
- Final `cargo test --workspace`: succeeded across all crates.
- Final `cargo clippy --all-targets --all-features -- -D warnings`: succeeded.
- Final `git diff --check`: succeeded.
- Final `git status --short --ignored -- local-data`: showed only `!! local-data/`.
...
- S20 adds a deterministic local fault-injection smoke that starts the actual `resume-daemon` binary, waits until a synthetic import task is durably claimed, kills the child process before worker/finalization, then verifies a later daemon binary invocation expires and recovers the lease.
- The pause marker and short import-lease override are compiled only for debug builds and are controlled by `RESUME_DAEMON_DEBUG_PAUSE_AFTER_CLAIM_FILE` and `RESUME_DAEMON_DEBUG_IMPORT_LEASE_MS`; release builds keep the normal five-minute lease path.
- The daemon still processes each start-of-run eligible task ID at most once, so a worker failure in the same `--once` run does not create a retry loop; each claimed task gets a fresh lease timestamp rather than reusing the run-start time.
- The subprocess test now wraps the child process in a guard so timeout or early-return paths kill and wait on the paused debug child.
- Subprocess output is checked to exclude local root paths, file names, raw fixture text, claim tokens, and local diagnostics.
- S20 does not complete P6. It is not restart/recovery soak, disk-full/file-lock fault injection, production observability, long-running daemon behavior, or real 100k/1M benchmark evidence.
