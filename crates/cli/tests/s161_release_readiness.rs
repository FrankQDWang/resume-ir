...
#[test]
fn release_readiness_rejects_release_publication_evidence_missing_prohibited_material_without_path_leaks(
) {
    let data_dir = temp_path("release-readiness-release-publication-missing-material-data");
    let evidence_dir = temp_path("release-readiness-release-publication-missing-material-reports");
    fs::create_dir_all(&evidence_dir).unwrap();
    let release_publication = evidence_dir.join("release-publication-evidence.json");
    fs::write(
        &release_publication,
        release_publication_evidence_manifest().replace(",\"raw_resume_data\"", ""),
    )
    .unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_resume-cli"))
        ...
        .output()
        .expect("reject release publication evidence missing prohibited material marker");

    assert!(!output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("release readiness evidence failed validation"));
    assert!(stderr.contains("GitHub Release publication automation evidence"));
    assert!(stderr
        .contains("release publication evidence blocked: prohibited_public_material is invalid"));
    assert!(!stdout.contains(path_str(&data_dir)));
    assert!(!stderr.contains(path_str(&data_dir)));
    assert!(!stdout.contains(path_str(&evidence_dir)));
    assert!(!stderr.contains(path_str(&evidence_dir)));
    assert!(!stdout.contains(path_str(&release_publication)));
    assert!(!stderr.contains(path_str(&release_publication)));
...
#[test]
fn release_readiness_rejects_github_publication_gate_unknown_planned_step_without_path_leaks() {
    let data_dir = temp_path("release-readiness-github-publication-gate-extra-step-data");
    let evidence_dir = temp_path("release-readiness-github-publication-gate-extra-step-reports");
    fs::create_dir_all(&evidence_dir).unwrap();
    let publication_gate = evidence_dir.join("github-release-publication-gate.json");
    fs::write(
        &publication_gate,
        github_release_publication_gate_manifest().replace(
            ...
            "\"gh_release_download_verify\",\"upload_private_logs\"]",
        ),
    )
    .unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_resume-cli"))
        ...
        .output()
        .expect("reject GitHub Release publication gate unknown planned step");
    ...
    assert!(!stdout.contains(path_str(&publication_gate)));
    assert!(!stderr.contains(path_str(&publication_gate)));
    assert!(!stdout.contains("upload_private_logs"));
    assert!(!stderr.contains("upload_private_logs"));
...
#[test]
fn release_readiness_rejects_github_publication_gate_missing_prohibited_material_without_path_leaks(
) {
    let data_dir = temp_path("release-readiness-github-publication-gate-missing-material-data");
    let evidence_dir =
        temp_path("release-readiness-github-publication-gate-missing-material-reports");
    fs::create_dir_all(&evidence_dir).unwrap();
    let publication_gate = evidence_dir.join("github-release-publication-gate.json");
    fs::write(
        &publication_gate,
        github_release_publication_gate_manifest().replace(",\"raw_resume_data\"", ""),
    )
    .unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_resume-cli"))
        ...
        .output()
        .expect("reject GitHub Release publication gate missing prohibited material marker");
    ...
    assert!(stderr.contains(
        "GitHub Release publication gate blocked: prohibited_public_material is invalid"
    ));
    assert!(!stdout.contains(path_str(&data_dir)));
    assert!(!stderr.contains(path_str(&data_dir)));
    assert!(!stdout.contains(path_str(&evidence_dir)));
...
#[test]
fn release_readiness_rejects_package_manifest_unknown_field_without_path_leaks() {
    let data_dir = temp_path("release-readiness-package-manifest-unknown-field-private-data");
    let evidence_dir =
        temp_path("release-readiness-package-manifest-unknown-field-private-reports");
    fs::create_dir_all(&evidence_dir).unwrap();
    let macos_package = evidence_dir.join("macos-package.json");
    fs::write(
        &macos_package,
        macos_package_manifest().replace(
            "\"packaging_status\":\"unsigned_dry_run\"",
            "\"packaging_status\":\"unsigned_dry_run\",\"diagnostic_note\":\"redacted\"",
        ),
    )
    .unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_resume-cli"))
        ...
        .output()
        .expect("run release readiness with unknown package manifest field");
...
#[test]
fn release_readiness_rejects_package_manifest_artifact_unknown_field_without_path_leaks() {
    let data_dir = temp_path("release-readiness-package-artifact-unknown-field-private-data");
    let evidence_dir =
        temp_path("release-readiness-package-artifact-unknown-field-private-reports");
    fs::create_dir_all(&evidence_dir).unwrap();
    let windows_package = evidence_dir.join("windows-package.json");
    fs::write(
        &windows_package,
        windows_package_manifest().replace(
            "\"kind\":\"msi\",\"file\":\"resume-ir-v0.0.0-windows.msi\"",
            "\"kind\":\"msi\",\"diagnostic_note\":\"redacted\",\"file\":\"resume-ir-v0.0.0-windows.msi\"",
        ),
    )
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_resume-cli"))
        ...
        .output()
        .expect("run release readiness with unknown package artifact field");
    ...
    assert!(stdout.is_empty());
    assert!(stderr.contains("release readiness evidence failed validation"));
    assert!(stderr.contains("Windows package manifest evidence"));
    assert!(stderr.contains("diagnostic_note is not allowed"));
    assert!(!stdout.contains(path_str(&data_dir)));
    assert!(!stderr.contains(path_str(&data_dir)));
    assert!(!stdout.contains(path_str(&evidence_dir)));
...
fn release_readiness_json_accepts_local_evidence_reports_but_keeps_external_blockers() {
    ...
    assert!(provided_labels.contains(&"OCR runtime manifest evidence"));
    assert!(!provided_labels.contains(&"OCR runtime manifest/dependency evidence"));
    let ocr_runtime_evidence = provided
        .iter()
        .find(|evidence| evidence["label"] == "OCR runtime manifest evidence")
    ...
    for evidence in provided {
        ...
        let expected_boundary = match label {
            "embedding model manifest evidence" | "OCR runtime manifest evidence" => {
...
#[test]
fn release_readiness_clears_ocr_runtime_distribution_with_matching_package_payload() {
    let data_dir = temp_path("release-readiness-ocr-package-private-data");
    let evidence_dir = temp_path("release-readiness-ocr-package-private-reports");
    fs::create_dir_all(&evidence_dir).unwrap();
    let ocr_engine_artifact = evidence_dir.join("reviewed-ocr-engine.bin");
    let ocr_renderer_artifact = evidence_dir.join("reviewed-ocr-renderer.bin");
    let ocr_manifest = evidence_dir.join("reviewed-ocr-manifest.json");
    let macos_package = evidence_dir.join("macos-package.json");
    write_reviewed_ocr_manifest(&ocr_engine_artifact, &ocr_renderer_artifact, &ocr_manifest);
    fs::write(&macos_package, macos_package_manifest_with_ocr_runtime()).unwrap();
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_resume-cli"))
        ...
        .output()
        .expect("run release readiness with matching OCR package payload");

    assert!(!output.status.success());
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    let report: serde_json::Value =
        serde_json::from_str(&stdout).expect("release readiness OCR package json report");
    let provided = report["provided_evidence"]
        .as_array()
        .expect("provided evidence");
    let provided_labels = provided
    ...
    assert!(provided_labels.contains(&"OCR runtime manifest/dependency evidence"));
    assert!(provided_labels.contains(&"macOS package manifest evidence"));
    let ocr_distribution = provided
        .iter()
        .find(|evidence| evidence["label"] == "OCR runtime manifest/dependency evidence")
        .expect("OCR runtime distribution evidence");
    assert_eq!(
        ocr_distribution["privacy_boundary"],
        "blocked_release_evidence_manifest"
    );
    assert!(ocr_distribution["detail"]
        .as_str()
        .expect("OCR runtime distribution detail")
        .contains("matching bundled runtime package payload"));
...
fn current_stage_evidence_manifest() -> String {
    let base = concat!(
        ...
        "{\"file\":\"doctor.out\",\"sha256\":\"3333333333333333333333333333333333333333333333333333333333333333\"},",
...
fn release_sbom_manifest() -> String {
    concat!(
        ...
        "{\"spdxElementId\":\"SPDXRef-DOCUMENT\",\"relationshipType\":\"DESCRIBES\",\"relatedSpdxElement\":\"SPDXRef-Runtime-tesseract\"}",
        "]",
        "}"
    )
    .to_string()
...
fn macos_package_manifest_with_ocr_runtime() -> String {
    concat!(
        "{",
        "\"schema_version\":\"release.macos_package.v1\",",
        "\"version\":\"v0.0.0\",",
        ...
        "\"runtime_bundle_manifest\":{\"file\":\"runtime-bundle-manifest.json\",\"sha256\":\"4444444444444444444444444444444444444444444444444444444444444444\",\"bytes\":404,\"schema_version\":\"release.runtime_bundle.v1\",\"runtime_distribution_mode\":\"bundled\"},",
        "\"components\":[",
        "{\"id\":\"fixture-tesseract\",\"kind\":\"ocr-engine\",\"file\":\"tesseract\",\"sha256\":\"f4c4eb4c45e595f803f076791dd942e6fd8bb93076207f8830ed6b8694f11e4a\",\"bytes\":101,\"license\":\"Apache-2.0\",\"source\":\"https://github.com/tesseract-ocr/tesseract\"},",
        "{\"id\":\"fixture-pdftoppm\",\"kind\":\"pdf-renderer\",\"file\":\"pdftoppm\",\"sha256\":\"571699d70504c3e505293c25953a85c38bdc8c13681aed7f7e3c4ce77fc8245f\",\"bytes\":202,\"license\":\"GPL-2.0-or-later\",\"source\":\"https://poppler.freedesktop.org/\"},",
        "{\"id\":\"fixture-eng-tessdata\",\"kind\":\"ocr-language-pack\",\"file\":\"eng.traineddata\",\"sha256\":\"f4c4eb4c45e595f803f076791dd942e6fd8bb93076207f8830ed6b8694f11e4a\",\"bytes\":303,\"license\":\"Apache-2.0\",\"source\":\"https://github.com/tesseract-ocr/tessdata\"}",
        "]",
        "},",
        "\"artifacts\":[",
        "{\"kind\":\"pkg\",\"file\":\"resume-ir-v0.0.0-macos.pkg\",\"sha256\":\"4444444444444444444444444444444444444444444444444444444444444444\",\"bytes\":404},",
