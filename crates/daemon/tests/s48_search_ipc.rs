...
fn unsafe_artifact_root_blocks_services_without_exiting_daemon() {
    ...
    assert_eq!(status.body["schema_version"], "daemon.status.v4");
    ...
    assert_eq!(next_status.body["schema_version"], "daemon.status.v4");
...
impl DaemonHarness {
    ...
    fn wait_for_core(&self, predicate: impl Fn(&str) -> bool) -> HttpResponse {
        ...
        while Instant::now() < deadline {
            ...
            assert_eq!(response.status_code, 200, "{}", response.raw);
            if response.body["schema_version"] == "daemon.status.v4"
